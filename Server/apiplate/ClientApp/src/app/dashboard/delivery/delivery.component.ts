...
export class DeliveryComponent implements OnInit {
  ...
  ascending = false;
  searchValue = "";
  constructor(private _service: DeliveryService,private modalService: FuiModalService) {
  ...
  openFormModal(driver:Delivery) {
          ...
          controls: [
            {
              ...
              width: "0px",
              value: driver ? driver.id : undefined
            },
            {
              title: "اسم المستخدم",
              name: "username",
              ...
              width: "50%",
              value: driver ? driver.username : undefined,
              validators: [
                Validators.required,
            ...
            },
            {
              ...
              width: "50%",
              value: driver ? driver.phone : undefined,
              validators:[
                Validators.required,
            ...
            },
            {
              ...
              width: "100%",
              value: driver ? driver.image : undefined,
      ...
      this.modalService.open(new FormBuilderModal(
        {
          title: driver ? "تعديل بيانات مستخدم" : "مستخدم جديد",
          controlGroups: form
        }
      )).onApprove((result) => {
        if(driver)
        this.update(result as Delivery); 
        else 
        this.create(result as Delivery); 
      
      });
     }
  create(driver:Delivery){
    this.DimLoading = true;
    this._service.post(driver).subscribe(res =>{
      this.initData();
      this.DimLoading = false;
      this.modalService.open(new MessageModal({
        title: "نجاح",
  ...
  update(delivery:Delivery){
    this.DimLoading = true;
    this._service.put(delivery).subscribe(res =>{
      this.initData();
      this.DimLoading = false;
      this.modalService.open(new MessageModal({
        title: "نجاج",
